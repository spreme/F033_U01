...
void send_JK_feed_info(unsigned char dpid,unsigned char value_h,unsigned short value_l)
{
    unsigned short send_len = 0;
    ...
    //
    send_len = set_wifi_uart_byte(send_len,0);
    send_len = set_wifi_uart_byte(send_len,value_h);
    send_len = set_wifi_uart_byte(send_len,value_l >> 8);
    send_len = set_wifi_uart_byte(send_len,value_l & 0xff);
...
//	#ifdef SEND_LAN_CONNECT_FUNC
//	printf("send ID:%d  value:%ld \r\n",dpid, value);
//	#endif
    wifi_uart_write_frame(STATE_UPLOAD_CMD, MCU_TX_VER, send_len);
